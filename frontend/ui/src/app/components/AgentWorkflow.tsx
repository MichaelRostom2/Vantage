              ...
              <div className="flex flex-col items-center gap-3 group shrink-0">
                <div className="relative">
                  <motion.div
                    ...
                    animate={{
                      backgroundColor: agent.status === 'done' ? '#10B981' :
                                     agent.status === 'active' ? '#6366F1' : 'rgba(226, 232, 240, 0.5)',
                  ...
                  {agent.status === 'active' && (
                    <>
                      <motion.div
                        className="absolute -inset-3 rounded-2xl bg-[#6366F1]/30 blur-lg z-10"
                        animate={{
                          opacity: [0.3, 0.6, 0.3],
                          scale: [1, 1.15, 1]
                        }}
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <span className={`text-xs font-bold whitespace-nowrap ${
                    ...
                    agent.status === 'active' ? 'text-[#6366F1]' : 'text-slate-900'
                  }`}>
                    {agent.name}
                  </span>
                </div>
                ...
                <div className="flex-1 h-1 mx-4 bg-gradient-to-r from-slate-200 to-slate-200 rounded-full relative overflow-hidden min-w-[40px]">
                  {agent.status === 'done' && (
                    <motion.div
                      ...
                      className="absolute inset-0 bg-gradient-to-r from-[#10B981] via-[#6366F1] to-[#8B5CF6] origin-left rounded-full"
                    />
                  )}
                  {agent.status === 'active' && (
                    <>
                      <motion.div
                        className="absolute inset-0 bg-gradient-to-r from-[#6366F1]/30 to-[#8B5CF6]/30 rounded-full"
                        animate={{ scaleX: [0, 1, 1, 0] }}
                        transition={{ duration: 2, repeat: Infinity }}
                      />
                      <motion.div
                        className="absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-[#6366F1] rounded-full shadow-lg"
                        style={{
